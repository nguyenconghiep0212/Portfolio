...
function init() {
  ...
  const sun = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: "#FFE600", wireframe: true })
  );
  ...
  Object.entries(planets).forEach((p) => {
    ...
    let pg = new THREE.SphereGeometry(size, 16, 16);
    let pm = new THREE.MeshToonMaterial({ color: color, wireframe: true });
